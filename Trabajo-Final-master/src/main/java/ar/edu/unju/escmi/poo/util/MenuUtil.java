...
public class MenuUtil{
		...
		if (totalMesas.isEmpty()) {
			...
			case 2:
				...
				if (encontrado == false) {
					...
					agencia.setCuit(cuit);
					
					System.out.println("Ingrese nombre:");
					agencia.setNombre(sc.nextLine());
					System.out.println("Ingrese email:");
					agencia.setEmail(sc.nextLine());
					...
					agencia.setTelefono(tel);

					personaDao.guardarPersona(agencia);
	...
	public void opcion2() {
		...
		do {
			...
			try {// se controlorÃ¡ que se busque la reserva y que su estado sea no pagado (false)
		...
		} while (auxiliar == null);// si termina este bucle, se encontrÃ³ la reserva no pagada (que seria auxiliar)
	...
	public void opcion3() {
		//se eliminan las reservas sin pagar por si estas fueran canceladas, no se pueden borrr las ya pagadas porque quedan como registro
		if(reservaDao.obtenerReservasSinPagar().size()>0) {
		boolean hayError = false;
		int aux = 0;
